...
{
	m_iterableRef->iterate_impl(make_composition(i_try,make_function([this](private_reference i_value) -> Return { return (iter::iterable_state::in_error(iter::ActionError::ShiftError)) ? reinterpret_cast<reference>(i_value) : eval(m_transform,i_value); })),i_initialAction);
}
template<typename PublicTraits, typename PrivateTraits, typename Return, typename Type>
...
{
	m_iterableRef->iterate_impl(make_composition(i_try,make_function([this](private_const_reference i_value) -> Return { return (iter::iterable_state::in_error(iter::ActionError::ShiftError)) ? reinterpret_cast<const_reference>(i_value) : eval(m_transform,const_cast<private_reference>(i_value)); })),i_initialAction);
}
template<typename PublicTraits, typename PrivateTraits, typename Return, typename Type>
