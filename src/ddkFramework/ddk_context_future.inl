...
context_future<T> context_future<Return>::then(const ddk::function<T(Return)>& i_callback) &&
{
	...
	{
		context_future<T> res(std::move(*this).future<Return>::async(i_callback, ddk::static_lent_cast<executor_context_interface>(ddk::lend(context))), std::move(ddk::promote_to_ref(context)));
