...
const const_forward_iterable<const int> naturals = view::constrain([](const int& i_value){ return i_value > 0; }) <<= detail::owned_iterable<const int,mpl::empty_type_pack,mpl::type_pack<agnostic_sink_action_tag<const int&>,forward_action_tag,begin_action_tag>>(deduce_iterable(integers_container));
const const_forward_iterable<const int> odd_naturals = view::constrain([](const int& i_value) { return i_value > 0; }) <<= detail::owned_iterable<const int,mpl::empty_type_pack,mpl::type_pack<agnostic_sink_action_tag<const int&>,forward_action_tag,begin_action_tag>>(deduce_iterable(odd_integers_container));
const const_forward_iterable<const int> even_naturals = view::constrain([](const int& i_value) { return i_value > 0; }) <<= detail::owned_iterable<const int,mpl::empty_type_pack,mpl::type_pack<agnostic_sink_action_tag<const int&>,forward_action_tag,begin_action_tag>>(deduce_iterable(even_integers_container));
