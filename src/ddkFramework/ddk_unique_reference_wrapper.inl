...
template<typename T>
unique_reference_wrapper<T>::unique_reference_wrapper(T* i_data,const tagged_pointer<unique_reference_counter>& i_refCounter)
: unique_pointer_wrapper<T>(i_data,i_refCounter)
{
	DDK_ASSERT(i_data != NULL,"Trying to construct non null reference from void pointer");
}
template<typename T>
unique_reference_wrapper<T>::unique_reference_wrapper(T* i_data,tagged_pointer<unique_reference_counter>&& i_refCounter)
: unique_pointer_wrapper<T>(i_data,std::move(i_refCounter))
{
	DDK_ASSERT(i_data != NULL,"Trying to construct non null reference from void pointer");
}
template<typename T>
